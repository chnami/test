 ...
 */
/*
 * 만일 HelloApp클래스에서 MessageBeanEn 클래스 대신 다른 클래스 (MessageBeanKo)를 사용할 경우에
 	(MessageBeanKo클래스는 MessageBeanEn클래스와 같은 기능을 수행하는 메서드가 존재합니다.)
 	이 글래스의 메서드는 MessageBeanEn클래스의 메서드와 다른 이름으로 사용되고 잇습니다.
 	그래서 MessageBeanKo클래스를 실행하기 위해서는 아래의  27-28라인 처럼 코드를 수정해야합니다.
